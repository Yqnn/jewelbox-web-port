...
export const promptPlayerName = (onSubmit: (playerName: string) => void) => {
  ...
  input.value = localStorage.getItem('jewelBoxPlayerName') ?? 'Player';
  input.focus();
  input.select();
  ...
  const submitHighScoreName = () => {
    const playerName =
      input.value && input.value.trim()
        ...
        : 'Anonymous';
    localStorage.setItem('jewelBoxPlayerName', playerName);
    onSubmit(playerName);
...
export const initHandlers = ({
  ...
  bindButton(getButton('mobileRotate'), () => onKeyDown('k'));
